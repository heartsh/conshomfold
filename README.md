# ConsFold Program, which Predicts RNA Single Secondary Structures to Consider Sparse RNA Structural Alignments
# Installation
...
Now you can install the ConsFold program as follows: 
```bash
$ cargo install consfold
```
Check if this program has been installed properly as follows:
```bash
$ phylofold # Its available command options will be displayed.
```
After the test, the figures shown in the paper of the ConsFold program can be reproduced:
```bash
$ cd src
